...
def _capture_screen_fast(max_width: int = 1024, max_height: int = 768, quality: int = 55, force: bool = False) -> tuple[bytes | None, str]:
    """Fast in-memory screen capture for continuous streaming.
    Returns (jpeg_bytes, 'image/jpeg') or (None, '') if the screen content hasn't changed.
    """
    ...
    try:
        img.thumbnail((max_width, max_height), PIL.Image.BILINEAR)

        if not force:
            try:
                # Perceptual diff check: 16x16 thumbnail hash
                small = img.resize((16, 16), PIL.Image.BILINEAR).convert("L")
                curr_hash = hash(small.tobytes())
                if _last_screen_hash == curr_hash:
                    return None, ""
        ...
        print(f"[Vision] Fast screen capture error: {e}")
        return None, ""
