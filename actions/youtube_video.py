...
def _scrape_first_video_url(query: str) -> str | None:
    ...
    # Clean query: strip trailing punctuation/extra dots/spaces
    clean_query = re.sub(r'[\.\?\!\,\"]+$', '', query).strip()
    if not clean_query:
        clean_query = query.strip()

    # Try standard search first (reliable for specific video names), then filtered
    for filter_param in ["", f"&sp={_YT_VIDEO_FILTER}"]:
        search_url = (
            f"https://www.youtube.com/results"
            f"?search_query={quote_plus(clean_query)}"
            f"{filter_param}"
        )
        ...
        try:
            ...
            print(f"[YouTube] ⚠️ scrape_first_video_url attempt failed: {e}")
