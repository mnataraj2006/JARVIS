...
class MainWindow(QMainWindow):
    ...
    _screen_stream_sig = pyqtSignal(bool)    # Screen watch state changed (thread-safe)
    ...
    def __init__(self, face_path: str):
        ...
        self.on_toggle_screen_stream = None # callable: () -> bool
        ...
        self._screen_stream_sig.connect(self._set_screen_stream_indicator)
        ...
        sc_screen = QShortcut(QKeySequence("F9"), self)
        sc_screen.activated.connect(self._toggle_screen_stream)

    def _toggle_screen_stream(self):
        if self.on_toggle_screen_stream and callable(self.on_toggle_screen_stream):
            active = self.on_toggle_screen_stream()
            self._set_screen_stream_indicator(active)

    def _set_screen_stream_indicator(self, active: bool):
        if hasattr(self, "_screen_watch_btn"):
            if active:
                self._screen_watch_btn.setText("👁 SCREEN: LIVE")
                self._screen_watch_btn.setStyleSheet(f"""
                    QPushButton {{
                        background: {C.PRI_GHO}; color: {C.ACC};
                        border: 1px solid {C.ACC}; border-radius: 4px; padding: 2px 7px;
                        font-weight: bold; font-family: 'Courier New'; font-size: 8pt;
                    }}
                    QPushButton:hover {{ color: #ffffff; border-color: {C.PRI}; }}
                """)
            else:
                self._screen_watch_btn.setText("👁 SCREEN [F9]")
                self._screen_watch_btn.setStyleSheet(f"""
                    QPushButton {{
                        background: transparent; color: {C.TEXT_DIM};
                        border: 1px solid {C.BORDER}; border-radius: 4px; padding: 2px 7px;
    ...
    def _build_header(self) -> QWidget:
        ...
        self._screen_watch_btn = QPushButton("👁 SCREEN [F9]")
        self._screen_watch_btn.setToolTip("Toggle Real-Time Screen Stream / Screen Watch (F9)")
        self._screen_watch_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._screen_watch_btn.setStyleSheet(f"""
            QPushButton {{
                background: transparent; color: {C.TEXT_DIM};
                border: 1px solid {C.BORDER}; border-radius: 4px; padding: 2px 7px;
            ...
            QPushButton:hover {{ color: {C.PRI}; border-color: {C.PRI_DIM}; }}
        """)
        self._screen_watch_btn.clicked.connect(self._toggle_screen_stream)
        lay.addWidget(self._screen_watch_btn)
    ...
    def _check_config(self) -> bool:
        ...
        try:
            ...
            if not key or "YOUR_GEMINI_API_KEY" in key or "\n" in key or len(key) < 20:
                return False
...
class JarvisUI:
    ...
    def set_screen_stream_active(self, active: bool):
        """Thread-safe: update the Screen Watch button/indicator."""
        self._win._screen_stream_sig.emit(active)

    @property
    def on_toggle_screen_stream(self):
        return self._win.on_toggle_screen_stream

    @on_toggle_screen_stream.setter
    def on_toggle_screen_stream(self, cb):
        self._win.on_toggle_screen_stream = cb
