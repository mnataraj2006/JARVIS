...
def _get_api_key() -> str:
    ...
    try:
        with open(API_CONFIG_PATH, "r", encoding="utf-8") as f:
            ...
            if "\n" in key or "YOUR_GEMINI_API_KEY" in key:
                return ""
            return key
    except Exception:
        return ""
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        "name": "send_message",
        "description": (
            "THE ONLY tool for sending text messages or chatting on WhatsApp, Telegram, Instagram, Signal, or Discord. "
            "Use whenever the user asks to message someone, text someone, or 'open WhatsApp, go to X's chat and tell Y'. "
            "NEVER use computer_control, open_app, or mouse clicks to open WhatsApp and type messages manually."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "receiver":     {"type": "STRING", "description": "Recipient contact name or phone number. ALWAYS in English alphabet (e.g. 'Jeya Saravanan'). NEVER in Tamil script."},
                "message_text": {"type": "STRING", "description": "The message to send. Default is in English unless user explicitly asked to write in Tamil."},
                "platform":     {"type": "STRING", "description": "Platform: WhatsApp (default), Telegram, etc."}
            },
    ...
    },
    {
        "name": "youtube_video",
        "description": (
            "Controls YouTube for playing new videos or summarizing. "
            "Use when user asks to search or play a YouTube video from scratch. "
            "Do NOT use if YouTube or a browser is already open on screen and the user asks to 'click that', 'click the video', or click a specific item — use computer_control(action='screen_click') instead."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
    ...
    {
        "name": "toggle_screen_stream",
        "description": (
            "Turns real-time continuous screen sharing/watch ON or OFF. "
            "When active, JARVIS continuously receives live screen frames as video. "
            "Use when the user asks to: watch screen, start screen sharing, "
            "look at what I'm doing, stop watching screen, or turn off screen share."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action": {
                    "type": "STRING",
                    "description": "start | stop | toggle | status"
                }
            },
            "required": ["action"]
        }
    ...
    },
    {
        ...
        "description": (
            ...
            "Use action='screen_click' with 'description' to click ANY visible video thumbnail, button, link, search bar, or element on screen (e.g. 'third video', 'video titled VJ Siddhu Vlogs', 'search button')."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action":      {"type": "STRING", "description": "type | smart_type | click | double_click | right_click | hotkey | press | scroll | move | copy | paste | screenshot | wait | clear_field | focus_window | screen_find | screen_click | random_data | user_data"},
                "text":        {"type": "STRING", "description": "Text to type or paste. MUST ALWAYS be in English (Latin alphabet) by default unless explicitly instructed to type in Tamil script."},
...
class JarvisLive:
    ...
    def __init__(self, ui: JarvisUI):
        ...
        self._screen_stream_active = False   # True when real-time screen stream is active
        self.ui.on_text_command   = self._on_text_command
        self.ui.on_remote_clicked = self._make_remote_key
        self.ui.on_interrupt      = self.interrupt
        self.ui.on_toggle_screen_stream = self.toggle_screen_stream
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            ...
            elif name == "toggle_screen_stream":
                act = args.get("action", "toggle")
                is_on = self.toggle_screen_stream(act)
                result = (
                    "Live Screen Watch is now ACTIVE. I can see your screen in real time. "
                    "Ask me anything about what you have open."
                    if is_on else
                    "Live Screen Watch is now OFF. Screen sharing stopped."
                )
    ...
    def toggle_screen_stream(self, action: str = "toggle") -> bool:
        act = action.lower().strip()
        if act in ("start", "on", "enable"):
            self._screen_stream_active = True
        elif act in ("stop", "off", "disable"):
            self._screen_stream_active = False
        elif act == "status":
            pass
        else:
            self._screen_stream_active = not self._screen_stream_active

        self.ui.set_screen_stream_active(self._screen_stream_active)
        state_str = "ACTIVE (Live Stream)" if self._screen_stream_active else "OFF"
        print(f"[Vision] 🖥️ Screen Watch: {state_str}")
        self.ui.write_log(f"SYS: Screen Watch {state_str} (Hotkey: F9)")
        return self._screen_stream_active

    async def _stream_screen(self):
        """Continuously captures and streams real-time screen frames to Gemini Live session when active."""
        print("[JARVIS] 🖥️ Screen stream task ready (Hotkey: F9 to toggle)")
        loop = asyncio.get_event_loop()
        from actions.screen_processor import _capture_screen_fast

        while True:
            try:
                if self._screen_stream_active and self.session and self.out_queue and not self._interrupted:
                    img_bytes, mime_t = await loop.run_in_executor(
                        None, lambda: _capture_screen_fast(max_width=1024, max_height=768, quality=55)
                    )
                    if img_bytes and self._screen_stream_active and self.out_queue:
                        try:
                            self.out_queue.put_nowait({"data": img_bytes, "mime_type": "image/jpeg"})
                ...
                print(f"[Vision] ⚠️ Screen stream loop warning: {e}")
    ...
    async def _send_startup_briefing(self) -> None:
        """
        Instant startup greeting:
        Greets the user, states system readiness, and recalls last session if available.
        No news fetching or external reading.
        """
        memory   = load_memory()
        ...
        p1 = (
            f"Greet the user warmly in your signature Tony Stark JARVIS style, mention it is {time_str}, and state that all systems are online and ready to assist.{session_clause} "
            f"Keep it to 1 or 2 concise sentences max. Do not mention news. Do not call any tools.{lang_clause}{name_clause}"
        )
    ...
    async def run(self):
        ...
        while True:
            try:
                # Wait if setup overlay is asking the user for API key
                while hasattr(self.ui, "_win") and not getattr(self.ui._win, "_ready", True):
                    self.ui.set_state("SLEEPING")
                    await asyncio.sleep(1)

                api_key = _get_api_key()
                if not api_key:
                    self.ui.write_log("ERR: API key missing — please configure your key.")
                    self.ui.set_state("SLEEPING")
                ...
                # Invalid API key / connection rejected — stop hammering the API, prompt re-configuration
                if "API key not valid" in err_str or "1007" in err_str or "400 Bad Request" in err_str or "InvalidMessage" in err_str:
                    self.ui.write_log("ERR: API key invalid or connection rejected — please re-enter your key.")
                    self.ui.set_state("SLEEPING")
                    self.ui.prompt_reconfig()
                    while hasattr(self.ui, "_win") and not getattr(self.ui._win, "_ready", True):
                        await asyncio.sleep(1)
