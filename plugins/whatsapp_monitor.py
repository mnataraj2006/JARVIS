...
# Try importing winrt (modern) or winsdk (legacy) for native Windows Notification reading
try:
    try:
...
PLUGIN = {
    ...
    "description": (
        "Turn on or off the background WhatsApp auto-responder for incoming notifications while away. "
        "Do NOT call this tool when user asks to read an existing message or reply to someone who messaged earlier."
    ),
    "parameters": {
        "type": "OBJECT",
        "properties": {
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    if status == "on":
        ...
        missing_libs = []
        if not _WINSDK_AVAILABLE:
            missing_libs.append("winrt-Windows.UI.Notifications")
        if not _GENAI_AVAILABLE:
